...
namespace App\Http\Controllers\OrganizationLeader\Appointment;
...
class leaderAppointmentController extends Controller
{
      public function index(){
        ...
        return view('organization-leader.appointment.leader-appointment', compact('ocassions'));
    }
    ...
    'ocassion' => 'required|integer',
         ...
         if( $gaga = Appointment::where('user_id', auth()->user()->id)->where('ocassion_id', $request->ocassion)->whereDate('start_time', Carbon::today())->first()){
                   return  to_route('leader.appointment')->with('messageerror', 'You already had an appointment for ocassion '. $gaga->ocassion->name);
    ...
    return  to_route('leader.appointment')->with('message', 'Appointment Succesful');
        }catch(\Exception $exception){
            return to_route('leader.appointment')->with('message', "Appointment Denied. Something went wrong try again". $exception->getMessage());
        }
    ...
    return  to_route('home.landing')->with('message', 'Appointment Succesful');
            ...
            return to_route('home.landing')->with('message', "Appointment Denied. Something went wrong try again");
        }
      ...
      public function clientHistory(){
        return view('organization-leader.appointment.leader-appointment-history');
    }
