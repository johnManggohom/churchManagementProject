        ...
        <div @click.away="open = false" class="flex flex-col flex-shrink-0 w-full text-gray-700 bg-white md:w-64 dark-mode:text-gray-200 dark:bg-white drop-shadow-lg" x-data="{ open: false }">
            <div class="flex flex-row items-center justify-between flex-shrink-0 px-8 py-4">
                ...
                <nav :class="{'block': open, 'hidden': !open}" class="flex-grow px-4 pb-4 md:block md:pb-0 md:overflow-y-auto">
                     ...
                     <x-admin-nav-link  href="{{  route('client.dashboard') }}" :active="request()->routeIs('client.dashboard')"> {{ __('Home') }}</x-admin-nav-link> 
     
                           <x-admin-nav-link  href="{{  route('client.user-profile')  }}" :active="request()->routeIs('client.user-profile')"> {{ __('User Organization') }}</x-admin-nav-link> 
                               <x-admin-nav-link  href="{{  route('client.schedule.index')  }}" :active="request()->routeIs('client.schedule.index')"> {{ __('Schedule') }}</x-admin-nav-link> 
                    ...
                    <div @click.away="open = false" class="relative" x-data="{ open: false }">
                        ...
                        <div x-show="open" x-transition:enter="transition ease-out duration-100" x-transition:enter-start="transform opacity-0 scale-95" x-transition:enter-end="transform opacity-100 scale-100" x-transition:leave="transition ease-in duration-75" x-transition:leave-start="transform opacity-100 scale-100" x-transition:leave-end="transform opacity-0 scale-95" class="absolute right-0 w-full mt-2 origin-top-right rounded-md shadow-lg">
                            <div class="px-2 py-2 bg-white rounded-md shadow dark-mode:bg-gray-700">
                                        <x-admin-nav-link  href="{{  route('client.appointment')  }}" :active="request()->routeIs('client.appointment*')"> {{ __('Book Appointment') }}</x-admin-nav-link> 
                                           <x-admin-nav-link  href="{{  route('client.appointment.history')  }}" :active="request()->routeIs('client.appointment.history')"> {{ __('Appointment History') }}</x-admin-nav-link> 
                            </div>
                        </div>
                    </div>
                </nav>
         ...
         <main class="w-full">
                        <header class="flex items-center justify-between px-6 py-3 bg-white ">
