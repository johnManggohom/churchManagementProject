 ...
 <div class="block p-6 rounded-lg shadow-lg bg-white  mt-5 w-full">
  <form action="{{ route('admin.adminEvents.update', $event->id) }}"  method="POST" enctype="multipart/form-data">
    ...
    <div class="grid grid-cols-2 gap-4">
      ...
      <input type="date" id="inputDateFrom" name="inputDateFrom" value="{{\Carbon\Carbon::createFromFormat('Y-m-d H:i:s', $event->EventDateFrom)->format('Y-m-d')}}" class="form-control  @error('inputDateFrom') is-invalid @enderror" 
    ...
    <div class="form-group mb-6">
       ...
       <input type="date" id="inputDateFrom" name="inputDateTo"   value="{{\Carbon\Carbon::createFromFormat('Y-m-d H:i:s', $event->EventDateTo)->format('Y-m-d')}}"  class="form-control  @error('inputDateFrom') is-invalid @enderror" 
						value="{{old('inputDateFrom')}}" >
						@error('inputDateFrom')
						<div class="invalid-feedback">{{$message}}</div>
							@enderror
  ...
  <div class="mb-3 w-full mr-2">
    <label for="formFile" class="form-label inline-block mb-2 text-gray-700">Image <span class=" ml-1 text-sm"><span class="mr-1">|</span>File type: jpeg.png,jpg</span></label>
    <input id="inputPictures" name="inputPictures" value="{{ $event->Picture }}"  accept="image/*" class="form-control
    block
    w-full
    px-3
    py-1.5
  ...
  <div class="col-9">
                        <img src="{{ asset('storage/images/'.$event->Picture) }}" class="img-fluid img-thumbnail" width="100" style="height:80px;">
					</div>
</div>

                             <label class="block">
                                <input type="checkbox" name="inputPost" {{ $event->Post == 1 ? 'checked' : ''}} />
