    <div>
            ...
            <nav class="relative w-full flex flex-wrap items-center justify-between mb-2 py-1 bg-white text-gray-500 hover:text-gray-700 focus:text-gray-700  navbar navbar-expand-lg navbar-light">
  <div class="container-fluid w-full flex flex-wrap items-center justify-between px-6">
    <nav class="bg-grey-light rounded-md w-full" aria-label="breadcrumb">
      <ol class="list-reset flex">
        <li><a href="{{ route('leader.dashboard.index') }}" class="text-gray-500 hover:text-gray-600 text-sm">Organization Members</a></li>
        <li><span class="text-gray-500 mx-2 text-sm">/ Accepted Members</span></li>
      </ol>
    </nav>
  </div>
</nav>  
            ...
            {{-- <div class="mt-1">
                        ...
                        <table class="w-full border-separate border-spacing-y-3 ">
                            ...
                            <tbody>
                                ...
                                <td class="p-2">{{  $employee->user->name}}  {{  $employee->user->middle_name}} {{  $employee->user->last_name}}</td>  
                                <td class="p-2">{{  $employee->user->email}}</td>                                 
                                    <td class="flex justify-center">
                                        <form  method="POST" action="{{ route('leader.dashboard.show.pendingRemove', $employee->id) }}" onsubmit="return confirm('are you sure')">
                                          @csrf
                                        <button type="submit" class="bg-red-500 p-2 text-white hover:shadow-lg text-xs font-thin">Remove</button>
                                        </form>
                                    </td>
                                </tr>
                                ...
                                @empty
                                   ...
                                   <tr class="bg-white text-center border-b text-xs text-gray-600 py-5">
                                <td class="p-2">No Accepted Members</td>  
